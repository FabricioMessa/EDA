...
int RTree::Search(const int a_min[2], const int a_max[2], vector<vector<pair<int, int>>> &a_results, Node *a_node)
{
	if (a_node == nullptr)
	{
		a_results.clear();
	...
	Rect rect;
	rect.m_min[0] = a_min[0];
	rect.m_min[1] = a_min[1];
	rect.m_max[0] = a_max[0];
	rect.m_max[1] = a_max[1];

	if (a_node->IsInternalNode())
	{
		for (int i = 0; i < a_node->m_count; ++i)
		{
			if (Overlap(&rect, &a_node->m_branch[i].m_rect))				  
				Search(a_min, a_max, a_results, a_node->m_branch[i].m_child);
		}
	}
	else
	{
